...
- (void)commonInitWithDataSource:(id <NYTPhotoViewerDataSource>)dataSource initialPhoto:(id <NYTPhoto> _Nullable)initialPhoto delegate:(nullable id <NYTPhotosViewControllerDelegate>)delegate {
    ...
    _singleTapGestureRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(doneButtonTapped:)];
...
- (void)updateOverlayInformation {
    ...
    BOOL captionViewHide = NO;
    if ([self.delegate respondsToSelector:@selector(photosViewController:captionViewHide:)]) {
        captionViewHide = [self.delegate photosViewController:self captionViewHide:self.currentlyDisplayedPhoto];
    }
    if (captionViewHide) {
        
    } else {
        self.overlayView.captionView = btn;
    }
}
...
- (void)btnAction:(UIButton *)btn {
    if ([self.delegate respondsToSelector:@selector(photosViewController:actionDownload:)]) {
        [self.delegate photosViewController:self actionDownload:@"234"];
    }
}
